                            ...
                            children: [
                              ...
                              widget.bookings.couponData == null
                                  ? GestureDetector(
                                      onTap: () async {
                                        ...
                                        if (couponController.text.isNotEmpty) {
                                          Either<CouponBaseModel, CouponModel> result = await applyCoupn(
                                            coupon: couponController.text,
                                            bookingId: widget.bookings.bookingDetail!.id!,
                                          );

                                          result.fold(
                                            (error) {
                                              // print('There is an error here...');
                                              toast(appStore.isArabic ? error.messageAr! : error.message!);
                                            },
                                            (success) {
                                              toast(appStore.isArabic ? 'تم تطبيق القسيمة بنجاح' : 'Coupn has been applied successfully');
                                              print('Everything is success... ${success.toJson()}');

                                              widget.bookings.couponData = CouponData.fromJson(success.toJson());
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: appStore.isDarkMode ? lightGray : context.primaryColor,
                                          borderRadius: BorderRadius.circular(12),
                                        ),
                                        child: Center(
                                          child: Text(
                                            language.lblApply,
                                            style: primaryTextStyle().copyWith(color: appStore.isDarkMode ? context.primaryColor : white),
                                          ),
                                        ),
                                      ),
                                    )
                                  : InkWell(
                                      ...
                                      child: CircleAvatar(
                                        radius: 16,
                                        backgroundColor: appStore.isDarkMode ? lightGray : context.primaryColor,
                                        child: Icon(
                                          Icons.close,
                                          size: 16,
