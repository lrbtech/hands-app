...
Future<List<AddressModel>> getAddressList({int? page, required List<AddressModel> addressData, Function(bool)? lastPageCallback}) async {
  try {
    List<dynamic> res = await handleResponse(await buildHttpResponse('user-address-book', method: HttpMethodType.GET));
    if (res.length > 0) {
...
Future<Map<String, dynamic>> saveAddress(Map request) async {
  return (await handleResponse(await buildHttpResponse('user-address-book', request: request, method: HttpMethodType.POST)));
}
