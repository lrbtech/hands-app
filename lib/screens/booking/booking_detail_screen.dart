...
class _BookingDetailScreenState extends State<BookingDetailScreen> {
  ...
  Widget _buildReasonWidget({required BookingDetailResponse snap}) {
    if ((([BookingStatusKeys.cancelled, BookingStatusKeys.rejected, BookingStatusKeys.failed].contains(snap.bookingDetail!.status)) && (snap.bookingDetail!.reason != null && snap.bookingDetail!.reason!.isNotEmpty)))
      return Container(
      ...
      if (storageStatus == PermissionStatus.granted) {
        try {
          ...
          if (response.statusCode == 200) {
            ...
            print('Download failed: Status code ${response.statusCode} and -> ${response.body}');
          }
    ...
    if (snap.bookingDetail!.status == BookingStatusKeys.pending)
      return Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (snap.bookingDetail!.status == BookingStatusKeys.waitingAdvancedPayment && (snap.service != null && snap.service!.isAdvancePayment) && (snap.bookingDetail!.paymentStatus == null || snap.bookingDetail!.paymentStatus != PAYMENT_STATUS_PAID)) Text(language.advancePaymentMessage, style: primaryTextStyle(color: redColor)) else Text(language.lblWaitingForProviderApproval, style: primaryTextStyle(color: redColor)),
          ],
        ),
      );
  ...
  Widget bookingIdWidget() {
    return Row(
      ...
      children: [
        ...
        Text('#' + widget.bookingId.validate().toString(), style: boldTextStyle(color: appStore.isDarkMode ? white : primaryColor, size: LABEL_TEXT_SIZE)),
      ],
    );
  }
  ...
  Widget counterWidget({required BookingDetailResponse value}) {
    if (value.bookingDetail!.isHourlyService && (value.bookingDetail!.status == BookingStatusKeys.inProgress || value.bookingDetail!.status == BookingStatusKeys.hold || value.bookingDetail!.status == BookingStatusKeys.complete || value.bookingDetail!.status == BookingStatusKeys.onGoing))
      return Column(
        children: [
          16.height,
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(language.lblID.toUpperCase(), style: boldTextStyle(size: 14, color: Color(0xFF6D7698))),
                ...
                if (bookingData.paymentMethod.validate().isNotEmpty)
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text(language.lblMethod, style: boldTextStyle(size: 14, color: Color(0xFF6D7698))),
                      Text(
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(language.lblStatus, style: boldTextStyle(size: 14, color: Color(0xFF6D7698))),
                    Text(
                ...
                if (bookingData.txnId.validate().isNotEmpty && (bookingData.paymentMethod != PAYMENT_METHOD_COD || bookingData.paymentMethod != PAYMENT_METHOD_FROM_WALLET))
                  Column(
                    children: [
                      ...
                      Row(
                        children: [
                          Text(language.transactionId, style: boldTextStyle(size: 14, color: Color(0xFF6D7698))),
                          8.width,
                          Row(
                            children: [
  ...
  Widget titleWidget({required String title, required String detail, bool isUrgent = false, bool isReadMore = false, required TextStyle detailTextStyle}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (data.bookingDetail!.addressDetails!.latitude != null && data.bookingDetail!.addressDetails!.longitude != null)
            AppButton(
              width: context.width(),
              color: primaryColor,
              child: Row(
                ...
                children: [
                  ...
                  10.width,
                  Text(
                    appStore.selectedLanguageCode == 'en' ? 'Show location on map' : 'عرض الموقع على الخريطة',
                    style: boldTextStyle(color: white),
                  )
                ],
              ),
              onTap: () {
                MapUtils.openMap(data.bookingDetail!.addressDetails!.latitude.toDouble(), data.bookingDetail!.addressDetails!.longitude.toDouble());
              },
            )
  ...
  void _handlePendingApproval({
    ...
    Map req = {
      ...
      BookingUpdateKeys.paymentStatus: val.bookingDetail!.paymentStatus.validate(),
    };
  ...
  Widget _action({required BookingDetailResponse bookingResponse}) {
    if ((bookingResponse.service != null && bookingResponse.service!.isAdvancePayment && bookingResponse.bookingDetail!.bookingPackage == null) && (bookingResponse.bookingDetail!.paymentStatus == null || (bookingResponse.bookingDetail!.paymentStatus == SERVICE_PAYMENT_STATUS_ADVANCE_PAID && bookingResponse.bookingDetail!.status == BookingStatusKeys.complete))) {
      return AppButton(
    ...
    } else if (bookingResponse.bookingDetail!.status == BookingStatusKeys.accept && bookingResponse.bookingDetail!.paymentId == null) {
      return AppButton(
        text: language.lblPayNow,
        textColor: Colors.white,
        color: Colors.green,
        onTap: () {
          PaymentScreen(bookings: bookingResponse, isForAdvancePayment: false).launch(
            context,
            duration: Duration.zero,
          );
        },
      ).paddingOnly(left: 16, right: 16, bottom: 16);
    } else if (bookingResponse.bookingDetail!.status == BookingStatusKeys.pending) {
      return AppButton(
      ...
      return Column(
        children: [
          AppButton(
            ...
            color: primaryColor,
            onTap: () {
              print('Location latitude is ${bookingResponse.bookingDetail!.addressDetails!.latitude.validate().toDouble()}');
              print('Location longitude is ${bookingResponse.bookingDetail!.addressDetails!.longitude.validate().toDouble()}');
              print('providerName is ${bookingResponse.bookingDetail?.providerName}');

              OrderTrackingScreen(
                bookingId: bookingResponse.bookingDetail!.id!,
                userLocation: LatLng(
                  bookingResponse.bookingDetail!.addressDetails!.latitude.validate().toDouble(),
                  bookingResponse.bookingDetail!.addressDetails!.longitude.validate().toDouble(),
                ),
      ...
      return 1 == 1
          ? Container(
              ...
              child: Text(appStore.selectedLanguageCode == 'en' ? 'Waiting until the service provider completes the work ...' : 'في انتظار العمل حتى يكتمل', style: boldTextStyle(), textAlign: TextAlign.center).center(),
            )
          : Row(
              children: [
    ...
    } else if (bookingResponse.bookingDetail!.status == BookingStatusKeys.rejected) {
      return (1 == 1)
          ? Container(
              color: white,
              child: Center(
                ...
                AppButton(
                  child: Row(
                    ...
                    children: [
                      ...
                      10.width,
                      Text(
                        appStore.selectedLanguageCode == 'en' ? 'Do you need help with this job ?' : 'هل تريد المساعدة في هذا العمل؟',
                        style: boldTextStyle(color: white),
                      )
                    ],
                  ),
                  ...
                  onTap: () async {
                    ...
                    String androidUrl = "whatsapp://send?phone=$contact&text=${(appStore.selectedLanguageCode == 'en' ? whatsappString : whatsappStringAr)}${Uri.encodeComponent(jobId)}";
                    String iosUrl = "whatsapp://wa.me/$contact/?text=${(appStore.selectedLanguageCode == 'en' ? whatsappString : whatsappStringAr)}${Uri.encodeComponent(jobId)}";
                    ...
                    try {
                      ...
                      toast(appStore.selectedLanguageCode == 'en' ? 'WhatsApp Is Not Installed' : 'الواتساب غير متوفر');
                    }
                    // String phoneNumber = "";
    ...
    } else if (bookingResponse.bookingDetail!.status == 'pending_approval_again') {
      return AppButton(
        text: appStore.isArabic ? 'موافقة' : 'Approve',
        textStyle: boldTextStyle(color: white),
        color: forestGreen,
        ...
        Stack(
          children: [
            AnimatedScrollView(
              ...
              listAnimationType: ListAnimationType.FadeIn,
              children: [
                (snap.data!.bookingDetail!.status == BookingStatusKeys.accept && snap.data!.bookingDetail?.paymentId == null)
                    ? Container(
                        // height: 100,
                        padding: EdgeInsets.all(16),
                        color: redColor.withOpacity(0.05),
                        ...
                        child: Row(
                          children: [
                            ...
                            5.width,
                            Text(
                              appStore.selectedLanguageCode == 'en' ? 'The request will be on hold until you pay !' : 'هذا الطلب سيكون مجمد حتى يتم الدفع',
                              style: boldTextStyle(color: redColor),
                            ),
                          ],
                        ),
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text('${getCategoryName(snap.data!.postRequestDetail!.category)}', style: boldTextStyle(color: appStore.isDarkMode ? white : primaryColor, size: LABEL_TEXT_SIZE)),
                        ],
                      ),
                      ...
                      Divider(height: 32, color: context.dividerColor),
                      Text("${language.addressTitle}", style: boldTextStyle(size: LABEL_TEXT_SIZE)),
                      8.height,
                      ...
                      GestureDetector(
                        onTap: () async {
                          String message = "${language.contactUsOnWhatsAppMessage}${snap.data!.bookingDetail!.id!}.. ";

                          String androidUrl = "whatsapp://send?phone=${appStore.helplineNumber.validate()}&text=$message";
                          String iosUrl = "whatsapp://wa.me/${appStore.helplineNumber.validate()}/?text=$message";
                          ...
                          try {
                            ...
                            toast(appStore.isArabic ? 'تطبيق الواتس آب غير مُثبَت على هذا الموبايل' : 'WhatsApp is not installed on this device.', bgColor: redColor);
                          }
                        },
                        child: SizedBox(
                          height: 50,
                      ...
                      if (snap.data!.bookingDetail!.status == BookingStatusKeys.rejected) 40.height,
            ...
            Positioned(
                ...
                right: 0,
                child: Padding(
                  padding: snap.data!.bookingDetail!.status == BookingStatusKeys.pendingApproval ? EdgeInsets.zero : const EdgeInsets.only(bottom: 16),
