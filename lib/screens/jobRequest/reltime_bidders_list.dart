...
class _BiddersListScreenState extends State<BiddersListScreen> {
  ...
  init({bool initState = false}) {
    final reference = database.ref().child('$JOB_REQUESTS/${widget.postJobData.id}/bidders/');
    ...
    if (dataBaseEvent == null) {
      dataBaseEvent = reference.onChildAdded.listen((event) async {
        ...
        final x = BidderData.fromJson(json.decode(json.encode(event.snapshot.value)));
        ProviderInfoResponse provider = await getProviderDetail(x.providerId.validate(), userId: appStore.userId.validate());
        x.provider!.providersServiceRating = provider.userData!.providersServiceRating;
  ...
  notifyBid() async {
    snackBar(context, title: appStore.selectedLanguageCode == 'en' ? 'New bid arrived' : 'وصل عرض جديد على طلبك', backgroundColor: greenColor, textColor: white);
    try {
  ...
  Future<void> afterAccept() async {
    ...
    try {
      ...
      await database.ref().child('$JOB_REQUESTS/${widget.postJobData.id}').remove();
      // finish(context);
      // finish(context);
    } catch (e) {
      print('--------->ERROR : $e');
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false,
      onPopInvoked: (_) {
        toast(appStore.selectedLanguageCode == 'en' ? 'Please select provider first' : 'الرجاء اختيار مقدم الخدمة اولا');
      },
      child: AppScaffold(
        appBarTitle: appStore.selectedLanguageCode == 'en' ? 'Available Providers' : 'مقدمي الخدمة المتاحين',
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                appStore.selectedLanguageCode == 'en' ? 'Available service providers.' : 'مقدمي الخدمة المتاحين',
                                style: primaryTextStyle(size: 12),
                              ),
                              Text(
                                appStore.selectedLanguageCode == 'en' ? 'Based on your request, below are all available providers who approved your scopes of work.' : 'بناءً على طلبك، فيما يلي جميع مقدمي الخدمة المتاحين الذين وافقوا على منطقتك.',
                                style: secondaryTextStyle(size: 10, weight: FontWeight.w100),
                              ),
                            ],
                          ),
                        ),
