...
class _MyPostDetailScreenState extends State<MyPostDetailScreen> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('${getCategoryName(data.category)}', style: boldTextStyle(), maxLines: 2, overflow: TextOverflow.ellipsis),
          if (data.title.validate().isNotEmpty) titleWidget(title: language.postJobTitle, detail: data.title.validate(), detailTextStyle: boldTextStyle(), isUrgent: data.isUrgentRequest),
            ...
            children: [
              ...
              if (data.timeSlot.validate().isNotEmpty)
                titleWidget(
                  ...
                  detail: appStore.selectedLanguageCode == 'en' ? data.timeSlot.validate() : data.timeSlotAr.validate(),
                  detailTextStyle: boldTextStyle(),
                  isReadMore: false,
                ),
            ],
        ...
        children: [
          ...
          if (data.address!.latitude != null && data.address!.longitude != null)
            AppButton(
              ...
              onTap: () {
                MapUtils.openMap(data.address!.latitude.toDouble(), data.address!.longitude.toDouble());
              },
            )
          // Text(data.status.validate() == JOB_REQUEST_STATUS_ASSIGNED ? language.jobPrice : language.estimatedPrice, style: secondaryTextStyle()),
          // 4.height,
                ...
                children: [
                  HorizontalList(
                    ...
                    itemBuilder: (context, index) {
                      return 1 == 1
                          ? GalleryComponent(images: data.attachmentsArray!.map((e) => e.url!).toList(), index: index)
                          : CachedImageWidget(
                              url: data.attachments.validate().isNotEmpty ? data.attachments![index].validate() : "",
                              fit: BoxFit.cover,
                              height: 50,
                              width: 50,
        ...
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            return BidderItemComponent(
              ...
              serviceId: postJobDetailResponse.postRequestDetail!.serviceId.validate(),
              afterAccept: () {},
  ...
  void bookServices({required PostJobDetailResponse postJobDetailResponse}) {
    if (postJobDetailResponse.postRequestDetail != null && postJobDetailResponse.postRequestDetail!.service.validate().isNotEmpty) {
      serviceId = postJobDetailResponse.postRequestDetail!.service!.first.id.validate();
    }
    ...
    Map request = {
      CommonKeys.id: "",
      PostJob.postRequestId: postJobDetailResponse.postRequestDetail!.id.validate(),
      CommonKeys.serviceId: serviceId,
      CommonKeys.providerId: postJobDetailResponse.postRequestDetail!.providerId.toString(),
      CommonKeys.customerId: appStore.userId.toString().toString(),
      CommonKeys.status: BookingStatusKeys.accept,
      CommonKeys.address: postJobDetailResponse.postRequestDetail!.address!.address.validate(),
      CommonKeys.date: postJobDetailResponse.postRequestDetail!.date.validate(),
      BookService.amount: postJobDetailResponse.postRequestDetail!.jobPrice.validate(),
      BookingServiceKeys.totalAmount: postJobDetailResponse.postRequestDetail!.jobPrice.validate(),
      BookingServiceKeys.type: BOOKING_TYPE_USER_POST_JOB,
      BookingServiceKeys.couponId: '',
      BookingServiceKeys.description: postJobDetailResponse.postRequestDetail!.description.validate(),
    };
          ...
          return Stack(
            children: [
              AnimatedScrollView(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  postJobDetailWidget(data: data.postRequestDetail!).paddingAll(10),
                  if (data.postRequestDetail!.addressDetails != null) addressDetailWidget(data: data.postRequestDetail!).paddingAll(10),
                  if (data.postRequestDetail!.service != null && data.postRequestDetail!.service!.isNotEmpty && data.postRequestDetail!.service!.first != null && data.postRequestDetail!.service!.first.attachments.validate().isNotEmpty) postJobServiceWidget(serviceList: data.postRequestDetail!.service.validate()),
                  ...
                  // else
                  bidderWidget(data.biderData.validate(), postJobDetailResponse: data),
                  10.height,
                ],
