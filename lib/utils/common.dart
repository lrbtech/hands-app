  ...
  if (isTime) {
    ...
    if (getStringAsync(DATE_FORMAT).validate().contains('dS')) {
      ...
      if (DateFormat('${getStringAsync(DATE_FORMAT)}', languageCode).format(parsedDateTime).contains('$day')) {
        return DateFormat('${getStringAsync(DATE_FORMAT).replaceAll('S', '')} ${showDateWithTime ? getStringAsync(TIME_FORMAT) : ''}', languageCode).format(parsedDateTime).replaceFirst('$day', '${addOrdinalSuffix(day)}');
      }
    }
...
String formatBookingDate(String? dateTime, {String format = DATE_FORMAT_1, bool isFromMicrosecondsSinceEpoch = false, bool isLanguageNeeded = false, bool isTime = false, bool showDateWithTime = false}) {
  final languageCode = isLanguageNeeded ? appStore.selectedLanguageCode : null;
  final parsedDateTime = isFromMicrosecondsSinceEpoch ? DateTime.fromMicrosecondsSinceEpoch(dateTime.validate().toInt() * 1000) : DateTime.parse(dateTime.validate());
...
Widget get trailing {
  return RotatedBox(quarterTurns: appStore.selectedLanguageCode == 'ar' ? 2 : 0, child: ic_arrow_right.iconImage(size: 16));
}
...
Future<void> setGeneralSetting(GeneralSettingModel generalSettingModel) async {
  ...
  print("Kareem SITE_DESCRIPTION_AR API = ${generalSettingModel.siteDescriptionAR}");
}
