...
Future<ConfigurationResponse> getAppConfigurations({bool isCurrentLocation = false, double? lat, double? long}) async {
  try {
    ...
    if (data.otherSettings != null) {
      ...
      otherSettingStore.setDisclimerText(data.otherSettings!.disclaimerText.validate(), data.otherSettings!.disclaimerTextAr.validate());
    ...
    if (data.countryList != null && data.countryList.validate().isNotEmpty) {
      ...
      if (data.countryList.validate().first.countryId.validate().toString() != appStore.countryId.toString()) appStore.setCurrencyCountryId(data.countryList.validate().first.countryId.validate().toString());
...
Future<DashboardResponse> userDashboard({bool isCurrentLocation = false, double? lat, double? long}) async {
  ...
  try {
    ...
    setValue(SITE_DESCRIPTION_AR, dashboardResponse.generalSetting?.siteDescriptionAR);
    setValue(SITE_DESCRIPTION, dashboardResponse.generalSetting?.siteDescription);
    ...
    appStore.setStoresUrls(
      ...
      providerPlaystoreUrl: dashboardResponse.appDownload?.provider_playstore_url,
...
void _performAdditionalProcessing(DashboardResponse dashboardResponse) async {
  ...
  if (dashboardResponse.countryList != null && dashboardResponse.countryList.validate().isNotEmpty) {
    if (dashboardResponse.countryList.validate().first.currencyCode.validate() != appStore.currencyCode) appStore.setCurrencyCode(dashboardResponse.countryList.validate().first.currencyCode.validate());
    if (dashboardResponse.countryList.validate().first.countryId.validate().toString() != appStore.countryId.toString()) appStore.setCurrencyCountryId(dashboardResponse.countryList.validate().first.countryId.validate().toString());
    if (dashboardResponse.countryList.validate().first.currencySymbol.validate() != appStore.currencySymbol) appStore.setCurrencySymbol(dashboardResponse.countryList.validate().first.currencySymbol.validate());
  }
...
Future<bool?> tipProvider(Map req) async {
  appStore.setLoading(true);
  try {
    var res = BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('tip-provider', method: HttpMethodType.POST, request: req)));
...
Future<BaseResponseModel> contactUs(Map request) async {
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('contact-us', request: request, method: HttpMethodType.POST)));
}
//endregion
...
Future<InvoiceResponseModel> sentInvoiceOnMail(Map request) async {
  return InvoiceResponseModel.fromJson(await handleResponse(await buildHttpResponse('download-invoice', request: request, method: HttpMethodType.POST)));
}
//endregion
